...
const TimelineNode: React.FC<TimelineNodeProps> = ({
  ...
  let boxClasses =
    "w-full md:w-auto max-w-[150px] md:max-w-[200px] cursor-pointer hover:shadow-md transition-shadow p-3 border border-gray-200 text-gray-900";
  const titleClasses = "font-medium line-clamp-2 text-sm";
  const dateClasses = "text-xs mt-2 text-gray-500 font-mono";
  const pillClasses =
    "inline-flex items-center border px-2.5 py-0.5 transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 mb-2 text-xs border-gray-200";
  ...
  const evidenceTypeDisplay =
    event.type === "evidence" && event.evidenceSourceType
  ...
  return (
    <div className={boxClasses} onClick={onClick}>
      {isFirstMention && (
        <div
          className={`${pillClasses} bg-green-50 text-green-700 border-green-200`}
        >
          First mention
        </div>
      )}
      {isLast && event.type === "evidence" && !isFirstMention && (
        <div
          className={`${pillClasses} bg-blue-50 text-blue-700 border-blue-200`}
        >
          Most recent
        </div>
      )}
