...
export default function PromiseModal({
  ...
  const sortedEvidence = [...evidenceItems].sort((a, b) => {
    const dateA =
      a.evidence_date instanceof Timestamp
        ? a.evidence_date.toMillis()
        : new Date(a.evidence_date).getTime();
    const dateB =
      b.evidence_date instanceof Timestamp
        ? b.evidence_date.toMillis()
        : new Date(b.evidence_date).getTime();
          ...
          <section className="border-t border-[#d3c7b9] pt-8">
            ...
            {sortedEvidence.length > 0 ? (
              <div className="relative border-l-2 border-[#d3c7b9] pl-6 space-y-6">
                {sortedEvidence.map((item: EvidenceItem) => (
                  <div key={item.evidence_id} className="relative">
                    ...
                    <div className="mb-1 text-sm font-medium text-[#8b2332]">
                    ...
                    {item.description_or_details && (
                      <p className="text-[#333333] mt-1 mb-2 text-sm">
                        {item.description_or_details}
                      </p>
                    )}
                    {item.source_url && (
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Type: {item.evidence_source_type}
                    </p>
              ...
              <p className="text-[#555555] italic">
                No timeline events recorded yet for this promise.
              </p>
            )}
          </section>
        </div>
