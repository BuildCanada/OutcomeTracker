...
EVIDENCE_ITEMS_COLLECTION = "evidence_items" # Using test collection
...
LLM_MODEL_NAME = os.getenv("GEMINI_MODEL_NAME_NEWS_PROCESSING", "models/gemini-2.5-flash-preview-05-20")
...
def build_news_gemini_prompt(raw_item_data, prompt_template: str) -> str:
    ...
    title_value = raw_item_data.get('title') or raw_item_data.get('title_raw', '')
    description_value = raw_item_data.get('description') or raw_item_data.get('summary_or_snippet_raw', '')
    
    return prompt_template.format(
        news_title=title_value,
        news_summary_snippet=description_value[:2000],
...
async def process_pending_raw_news(db_client, dry_run=False, output_to_json=False, json_output_dir=None, force_reprocessing=False, start_date_filter_dt=None, end_date_filter_dt=None, limit=None):
    ...
    try:
        ...
        if force_reprocessing:
            ...
            logger.info("Querying for 'pending_evidence_creation' and error status items in date range.")
            # Query for pending items and error items that can be retried
            error_statuses = ["error_missing_fields", "error_llm_processing", "error_llm_missing_summary", "error_processing_script"]
            all_target_statuses = ["pending_evidence_creation"] + error_statuses
            query = query.where(filter=firestore.FieldFilter("evidence_processing_status", "in", all_target_statuses))
        ...
        for raw_item_doc_snapshot in pending_item_docs:
            ...
            # Check if item should be processed (pending or error statuses)
            current_status = raw_item_data.get("evidence_processing_status")
            processable_statuses = ["pending_evidence_creation", "error_missing_fields", "error_llm_processing", "error_llm_missing_summary", "error_processing_script"]
            
            if not force_reprocessing and current_status not in processable_statuses:
                logger.info(f"Item {raw_item_id} status is '{current_status}'. Skipping.")
                continue
            title_for_log = raw_item_data.get('title') or raw_item_data.get('title_raw', '')
            logger.info(f"Processing raw news item: {raw_item_id} - {title_for_log}")
            processed_count +=1

            try:
                # Check for title in either field name
                title_value = raw_item_data.get("title") or raw_item_data.get("title_raw")
                if not title_value or not raw_item_data.get("publication_date") or not raw_item_data.get("source_url"):
                    ...
                    if not dry_run:
                        await asyncio.to_thread(db_client.collection(RAW_NEWS_RELEASES_COLLECTION).document(raw_item_id).update, {
                            ...
                            "llm_model_name_last_attempt": LLM_MODEL_NAME
                        })
                    error_count +=1
                    continue
                ...
                if not gemini_result_dict:
                    logger.error(f"LLM analysis failed for {raw_item_id} (Model: {model_name_from_call}).")
                    if not dry_run:
                         await asyncio.to_thread(db_client.collection(RAW_NEWS_RELEASES_COLLECTION).document(raw_item_id).update, {
                ...
                if not timeline_summary:
                    logger.error(f"LLM result for {raw_item_id} missing timeline_summary (Model: {model_name_from_call}).")
                    if not dry_run:
                        await asyncio.to_thread(db_client.collection(RAW_NEWS_RELEASES_COLLECTION).document(raw_item_id).update, {
                ...
                if potential_relevance_score.lower() == "low":
                    logger.info(f"Skipping item {raw_item_id} due to low relevance score ('{potential_relevance_score}') (Model: {model_name_from_call}).")
                    if not dry_run:
                        await asyncio.to_thread(db_client.collection(RAW_NEWS_RELEASES_COLLECTION).document(raw_item_id).update, {
                ...
                evidence_item_data = {
                    ...
                    "description_or_details": one_sentence_description_llm, # Created by LLM
