...
export default async function SectionPage({ params }) {
  ...
  const sectionPromises = promises.filter(
    (promise) => promise.section.toLowerCase() === section.toLowerCase(),
  );
  ...
  const statusBadges = {
    ...
    "Partially Complete": (
      <Badge className="bg-yellow-500">Partially Complete</Badge>
    ),
    ...
    <main className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-xl text-muted-foreground mt-2">
          Tracking progress on {sectionPromises.length} promises across{" "}
          {Object.keys(subsectionGroups).length} subsections
        </p>
      </div>
        ...
        {Object.entries(subsectionGroups).map(([subsection, promises]) => (
          <div key={subsection} className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link
                href={`/sections/${section}/${encodeURIComponent(subsection.toLowerCase())}`}
              >
                  ...
                  <Card key={promise.source_ref} className="overflow-hidden">
                    <CardHeader>
                      <CardTitle className="text-lg flex justify-between">
                        ...
                        <div className="ml-4 flex-nowrap whitespace-nowrap">
                          {statusBadges[randomStatus]}
                        </div>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="flex flex-col gap-2">
                        {promise.promise_details.map((d: str) => {
                          return <p className="text-sm">{d}</p>;
                        })}
                        <div className="text-sm text-muted-foreground">
                          Source: Page {promise.source_page}, Ref{" "}
                          {promise.source_ref}
                        </div>
                      </div>
                    </CardContent>
