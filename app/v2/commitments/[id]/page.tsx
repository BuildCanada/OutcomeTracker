...
// ── Types matching the API response from GET /commitments/:id ───────────────
...
export default function CommitmentDetailPage() {
  ...
  const successCriteria = criteria.filter(
    (cr: Criterion) => cr.category === "success",
  );
        ...
        {criteria.map((cr) => (
          <li key={cr.id} className="flex items-start gap-2.5">
            <div className="flex-shrink-0 mt-0.5">
              ...
              ) : cr.status === "partially_met" ? (
                <div className="w-5 h-5 border-2 border-yellow-400 bg-yellow-50 flex items-center justify-center overflow-hidden relative">
                  <div className="absolute inset-0 bg-yellow-400 w-1/2" />
                  <svg
                    className="w-3.5 h-3.5 text-yellow-700 relative z-10"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            <div>
              ...
              {cr.status === "partially_met" && (
                <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-yellow-100 text-yellow-800 mt-1">
                  Partially met
                </span>
              )}
            </div>
